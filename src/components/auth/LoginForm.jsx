...
const LoginForm = ({ onToggle }) => {
  ...
  const fillDemo = (role) => {
    setRole(role)
    setEmail(role === 'admin' ? 'adii@gmail.com' : role === 'student' ? 'student1@example.com' : 'teacher1@example.com')
    setPassword(role === 'admin' ? 'adii001' : 'password123')
    setErrors({})
  }
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
        {[
          ...
          { role: 'admin', label: 'Admin', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg> },
        ].map(d => (
